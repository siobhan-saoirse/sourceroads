...
#define SOCKET_URL "http://localhost:2634/update"
...
#define TRACE_URL2 "http://localhost:7001/traceattacks"
...
{
    Handle hGameData = LoadGameConfigFile("socketio.games");
    if (hGameData == null)
        SetFailState("Failed to load socket.io gamedata");

    StartPrepSDKCall(SDKCall_Entity);
    PrepSDKCall_SetFromConf(hGameData, SDKConf_Virtual, "CTFWeaponBase::GetWorldModel");
    PrepSDKCall_SetReturnInfo(SDKType_String, SDKPass_Pointer, 0, VENCODE_FLAG_COPYBACK);
    gH_GetWorldModel = EndPrepSDKCall();
    if (gH_GetWorldModel == INVALID_HANDLE)
...
// Hook called when a player spawns
...
public SDKHooks_OnPreThink(iClient)
{
    ...
    for (int client = 1; client <= MaxClients; client++)
    {
        ...
        {
            SDKCall(gH_GetWorldModel, weapon, weaponModel, sizeof(weaponModel));
        }
