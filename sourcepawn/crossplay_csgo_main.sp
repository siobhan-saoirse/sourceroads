...
public Action Timer_SendAllPlayers(Handle timer)
{
    ...
    for (int client = 1; client <= MaxClients; client++)
    {
        ...
        GetEntPropString(client, Prop_Data, "m_ModelName", m_plrModelName, sizeof(m_plrModelName));
        if (StrContains(m_plrModelName, "custom", false) == -1) continue;
        float pos[3], ang[3];
        ...
        Format(name2, sizeof(name2), "[CS:GO] %s", name);
        ...
        if (GetClientTeam(client) == 3) {
            Format(entry, sizeof(entry),
                ...
                ang[0], ang[1], ang[2],
                vel[0], vel[1], vel[2],
                "models/player/ct_sas.mdl", skin, anim, "models/empty.mdl", scale, sceneName, curSeq
            );
            ...
            Format(entry, sizeof(entry),
                ...
                ang[0], ang[1], ang[2],
                vel[0], vel[1], vel[2],
                "models/player/t_phoenix.mdl", skin, anim, "models/empty.mdl", scale, sceneName, curSeq
            );
        } 
