...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
    ...
    if (IsValidClient(victim) && IsValidClient(attacker) && GetClientTeam(victim) == GetClientTeam(attacker)) return Plugin_Continue;
    ...
    if (StrContains(classname, "generic_actor", false) != -1) {
        damage = -1.0;
