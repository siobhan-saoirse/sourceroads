...
#define TF_UPDATE_URL "http://localhost:2635/update_css"
#define TERROR_UPDATE_URL "http://localhost:6003/update"
#define UPDATE_PROPS_URL "http://localhost:2634/update_props"
#define UPDATE_SOUND_URL "http://localhost:6000/sound"
...
public void OnMapStart()
{
    ...
    CreateTimer(0.03, Timer_SendUpdates, _, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
    CreateTimer(0.03, Timer_SendUpdatesTF, _, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
    CreateTimer(0.03, Timer_SendUpdatesTERROR, _, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
    ...
    while (pos < bodyLen)
    {
        ...
        if (idx == -1)
        {
            ...
            if (animation > 0)
            {
                ...
                if (curSeq != animation)
                {
                    SetEntProp(ent, Prop_Send, "m_nSequence", animation);
                    SetEntPropFloat(ent, Prop_Send, "m_flPlaybackRate", 1.0);
                }
            }
