...
#define TERROR_UPDATE_URL "http://localhost:2638/update"
#define CSS_UPDATE_URL "http://localhost:2635/update_css"
#define UPDATE_PROPS_URL "http://localhost:2635/update_props"
#define UPDATE_SOUND_URL "http://localhost:6002/sound"
#define SOUND_URL_1 "http://localhost:2637/sound"
#define SOUND_URL_2 "http://localhost:6004/sound"
#define SOUND_URL_3 "http://localhost:2639/sound"
...
float gPlayerLastSeen[MAX_PLAYERS];
...
public Action Timer_SendUpdatesTERROR(Handle timer)
{
    ...
    hRequest = SteamWorks_CreateHTTPRequest(k_EHTTPMethodGET, CSS_UPDATE_URL);
    if (hRequest != INVALID_HANDLE)
    {
        SteamWorks_SetHTTPRequestHeaderValue(hRequest, "Content-Type", "application/json");
        SteamWorks_SetHTTPCallbacks(hRequest, OnHTTPResponse4);
        SteamWorks_SendHTTPRequest(hRequest);
    }
...
public int OnHTTPResponse(Handle hRequest, bool bFailure, bool bRequestSuccessful, EHTTPStatusCode eStatusCode, any data)
{
    ...
    while (pos < bodyLen)
    {
        ...
        if (idx == -1)
        {
            ...
            if (idx == -1)
            {
                PrintToServer("[SYNC] No free slot for player '%s'", name);
                continue;
            }
            ...
            // Set the model (updates existing or new entity)
            ...
            gPlayerEnts[ent] = ent;
            activePlayers[ent] = true;
            gPlayerLastSeen[idx] = GetEngineTime();
        }
        else
        {
            ...
            if (IsValidEntity(ent)) {
                ...
                if (!IsValidEntity(ent) || IsValidEntity(ent) && StrContains(classname, "prop_dynamic_override", false) == -1)
                {
                    gPlayerEnts[idx] = 0;
                    continue;
                }
    ...
    const float PLAYER_TIMEOUT = 2.0; // seconds without update before removal
    // cleanup player slots 
    for (int i = 0; i < MAX_PLAYERS; i++)
    {
        if (gPlayerEnts[i] != 0)
        {
            // Check if player hasn't been updated recently
            if (!activePlayers[i] && (now - gPlayerLastSeen[i] > PLAYER_TIMEOUT))
            {
    ...
    if (hRequest != INVALID_HANDLE) CloseHandle(hRequest); // safely closed
    return 0;
}
...
public int OnHTTPPropsResponse(Handle hRequest, bool bFailure, bool bRequestSuccessful, EHTTPStatusCode eStatusCode, any data)
{
    ...
    while (start < propsLen)
    {
        ...
        if (pidx == -1)
        {
            ...
            if (pidx == -1)
            {
                PrintToServer("[PROPS] No free prop slot for id '%s'", id);
                continue;
            }
            ...
            if (pent == 0)
            {
                ...
                SetEntityModel(pent, "models/characters/hostage_01.mdl");
...
public Action Timer_CheckSounds(Handle timer)
{
    ...
    {
        SteamWorks_SetHTTPRequestHeaderValue(hRequest, "Content-Type", "application/json");
        SteamWorks_SetHTTPCallbacks(hRequest, OnHTTPSoundResponse2);
        SteamWorks_SendHTTPRequest(hRequest);
    }
    hRequest = SteamWorks_CreateHTTPRequest(k_EHTTPMethodGET, SOUND_URL_4);
    if (hRequest != INVALID_HANDLE)
    {
        SteamWorks_SetHTTPRequestHeaderValue(hRequest, "Content-Type", "application/json");
        SteamWorks_SetHTTPCallbacks(hRequest, OnHTTPSoundResponse);
...
public int OnHTTPSoundResponse(Handle hRequest, bool bFailure, bool bRequestSuccessful, EHTTPStatusCode eStatusCode, any data)
{
    ...
    while (start < bodyLen)
    {
        ...
        pos[0] = ExtractJSONFloat(entry, "x");
        pos[1] = ExtractJSONFloat(entry, "y");
        pos[2] = ExtractJSONFloat(entry, "z");
...
public int OnHTTPSoundResponse2(Handle hRequest, bool bFailure, bool bRequestSuccessful, EHTTPStatusCode eStatusCode, any data)
{
    if (bFailure || !bRequestSuccessful || eStatusCode != k_EHTTPStatusCode200OK)
        return 0;
    ...
    while (start < bodyLen)
    {
        ...
        } else if (StrContains(sound, "weapons/", false)) {
            ReplaceString(sound, sizeof(sound), ")","~");
        }
        int tbl = FindStringTable("soundprecache");
        if (tbl != INVALID_STRING_TABLE)
        {
        ...
        if (StrContains(sound, "vo/", false) != -1) {
            ...
            EmitAmbientSound(sound, pos, SOUND_FROM_WORLD, 150, _, 1.0, pitch);
        }
    }
...
public int OnHTTPResponse4(Handle hRequest, bool bFailure, bool bRequestSuccessful, EHTTPStatusCode eStatusCode, any data)
{
    if (bFailure || !bRequestSuccessful || eStatusCode != k_EHTTPStatusCode200OK)
    {
        PrintToServer("[SYNC] HTTP request failed or returned non-200: failure=%d success=%d code=%d", bFailure, bRequestSuccessful, eStatusCode);
